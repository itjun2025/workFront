window.onload = ()=>{
    ...
    // 이벤트가 발생한 요소는
    // 내부적으로 이벤트 발생정보를 담고 있는 객체를 매개변수로 전달한다.
    ...
    btn1.onclick = function(e){
        ...
        console.log('ctrlKey',e.ctrlKey); // ctrl키가 눌렸는지
        console.log('shiftKey',e.shiftKey); // shift키가 눌렸는지
    ...
    // 고전방식으로 이벤트를 적용할 경우
    // 속성에 값을 주는것이기 때문에 중복적용 불가능 => 덮어쓰기가된다
    btn1.onclick= (e)=>{
        // 화살표 함수에서는 this가 없음
        // 상위요소의 this를 찾아서 반환
    ...
    btn3.addEventListener('mouseleave',(d)=>{
        d.target.style.backgroundColor = 'green';
    });
...
// 이벤트 핸들러
// 이벤트 처리기 - 이벤트가 발생했을떄 처리하는 함수
function clickEventHandler(e){
    console.log('표준이벤트 모델')
}

// 2) 인라인 방식에 이벤트가 발생한 요소를 황니하는 방법
function test(e){
    // 매개값으로 이벤트 객체를 가져올 수 없다!!
    //console.log('e',e); // undefined
    //console.log(this)  // window 객체로 나옴
